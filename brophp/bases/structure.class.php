<?php
	...
	class Structure {
		...
		static function runtime(){
			$dirs=array(
					RUNTIME."runtime/cache/",   //系统的缓存目录
					RUNTIME."runtime/cache/".TPLSTYLE,   //系统的缓存目录
					RUNTIME."runtime/comps/",   //模板的组合文件
					RUNTIME."runtime/comps/".TPLSTYLE,   //模板的组合文件
					RUNTIME."runtime/comps/".TPLSTYLE."/".TMPPATH,   //模板的组合文件
					RUNTIME."runtime/data/",    //数据表的结构文件
					RUNTIME."runtime/controls/",
					RUNTIME."runtime/controls/".TMPPATH,
					RUNTIME."runtime/models/",
					RUNTIME."runtime/models/".TMPPATH
				);
		...
		static function create(){
			self::mkdir(array(RUNTIME."runtime/"));
			//文件锁，一旦生成，就不再创建
			$structFile=RUNTIME."runtime/".str_replace("/","_",$_SERVER["SCRIPT_NAME"]);  //主入口文件名
...
<?php
	class Index {
		function index(){
			echo "<b>欢迎使用《细说PHP》中的BroPHP框架2.0, 第一次访问时会自动生成项目结构：</b><br>";
			echo '<pre>';
...
st;
		...
		static function model($className, $app){
			$driver="D".DRIVER; //父类名
			$path=RUNTIME."runtime/models/".TMPPATH;
			if($app==""){
